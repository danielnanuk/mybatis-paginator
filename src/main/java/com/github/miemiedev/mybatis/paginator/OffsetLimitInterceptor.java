...
public class OffsetLimitInterceptor implements Interceptor {
    ...
    public Object intercept(final Invocation invocation) throws Throwable {
        ...
        if (pageBounds.getOffset() == RowBounds.NO_ROW_OFFSET
            ...
            if (retVal instanceof PageList) {
                PageList<?> pageList = (PageList<?>) retVal;
                if (pageList.getPaginator() == null) {
                    return new PageList<>(pageList, new Paginator(1, Integer.MAX_VALUE, pageList.size()));
                }
            }
            return retVal;
        }
        ...
        Boolean async = Optional.ofNullable(pageBounds.getAsyncTotalCount()).orElse(asyncTotalCount);
        Future<List<?>> listFuture = call(() -> (List<?>) invocation.proceed(), async);
        ...
        if (pageBounds.isContainsTotalCount()) {
            ...
            Future<Paginator> countFuture = call(countTask, async);
